...
add_shortcode('button', 'shortcode_button');
	function shortcode_button($atts, $content = null) {
		$atts = shortcode_atts(
			...
			return '<span><a href="' . $atts['link'] . '" target="' . $atts['target'] . '" class="progression-button ' . $atts['size'] .' '. $atts['color'] . '">' .do_shortcode($content). '</a></span>';
	}
...
function jqtools_tab_group( $atts, $content ){
	...
	if( is_array( $GLOBALS['tabs'] ) ){
		...
		foreach( $GLOBALS['tabs'] as $tab ){
			...
			if($counter == 1) {
				$panes[] = '<div id="tab'.$counter.'" class="progression-div">'.$tab['content'].'</div>';
			} else {
				$panes[] = '<div id="tab'.$counter.'" class="progression-div">'.$tab['content'].'</div>';
			}
			$counter++;
