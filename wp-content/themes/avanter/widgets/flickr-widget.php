...
class Flickr_Widget extends WP_Widget {
	...
	public function __construct()
		...
		parent::__construct('flickr-widget', 'Progression: Flickr', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		if($screen_name) { ?>
			...
			jQuery(document).ready(function($) {	 			   
				...
				$.getJSON("//api.flickr.com/services/feeds/photos_public.gne?id=<?php echo $screen_name; ?>&lang=en-us&format=json&jsoncallback=?", displayImages);  //YOUR IDGETTR GOES HERE
				function displayImages(data){																															   
					...
					$.each(data.items, function(i,item){
						...
						if (iCount > iStart && iCount < (iStart + <?php echo $number + 1; ?>)) {
	...
	function form($instance)
	{
		...
		<p>
			...
			<p>
				<label for="<?php echo $this->get_field_id('number'); ?>">Number of flickr image to show:</label>
